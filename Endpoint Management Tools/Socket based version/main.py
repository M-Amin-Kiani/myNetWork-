import psutil
import os
import time
import socket
import threading
import sys

sys.stdout.reconfigure(encoding='utf-8')  # برای رفع مشکل Unicode در ویندوز

HOST = socket.gethostbyname(socket.gethostname())
PORT = 0  # مقدار 0 یعنی سیستم خودش پورت آزاد را انتخاب کند

def alert_cpu_usage(client_ip, client_port):
    """مانیتورینگ مصرف CPU و ارسال هشدار از طریق UDP"""
    threshold = 80  
    check_interval = 10  
    with socket.socket(socket.AF_INET, socket.SOCK_DGRAM) as udp_socket:
        while True:
            cpu_usage = psutil.cpu_percent(interval=1)
            if cpu_usage <= threshold:
                message = f"⚠️ Warning: High CPU Usage! ({cpu_usage}%)"
                udp_socket.sendto(message.encode('utf-8'), (client_ip, client_port))
                print(f"[UDP] Sent warning to {client_ip}:{client_port}: {message}")
            time.sleep(check_interval)

def get_system_usage():
    """دریافت وضعیت CPU و RAM"""
    memory = psutil.virtual_memory()
    cpu_percent = psutil.cpu_percent(interval=1)
    return f"CPU: {cpu_percent}% | RAM: {memory.used // (1024*1024)}MB / {memory.total // (1024*1024)}MB"

def get_running_processes_count():
    """دریافت تعداد پردازش‌های فعال"""
    return f"Running Processes: {len(psutil.pids())}"

def restart_system():
    """ری‌استارت کردن سیستم"""
    os.system("shutdown /r /t 1" if os.name == "nt" else "sudo reboot")
    return "Restarting system..."

def handle_client(conn, addr):
    """مدیریت اتصال کلاینت"""
    try:
        data = conn.recv(1024).decode().strip()
        print(f"[TCP] Received client info: {data}")

        if ':' in data:
            client_ip, client_port = data.split(':')
            client_port = int(client_port)
        else:
            print("[ERROR] Invalid client info format!")
            conn.close()
            return

        monitoring_thread = threading.Thread(target=alert_cpu_usage, args=(client_ip, client_port), daemon=True)
        monitoring_thread.start()

        while True:
            command = conn.recv(1024).decode().strip()
            if not command:
                break

            if command == "ram":
                response = get_system_usage()
            elif command == "processes_count":
                response = get_running_processes_count()
            elif command == "restart":
                response = restart_system()
            elif command == "cpu_alert":
                cpu_percent = psutil.cpu_percent(interval=1)
                if cpu_percent <= 80:
                    response = f"⚠️ Warning: High CPU Usage! ({cpu_percent}%)"
                else:
                    response = f"CPU Usage: {cpu_percent}%"
            else:
                response = "[ERROR] Invalid command"

            print(f"[TCP] Sending response to {addr}: {response}")
            conn.sendall(response.encode('utf-8'))
    except Exception as e:
        print(f"[ERROR] Client {addr}: {e}")
    finally:
        conn.close()

def tcp_management():
    """مدیریت سرور TCP"""
    with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
        s.bind((HOST, PORT))
        s.listen()
        actual_port = s.getsockname()[1]
        print(f"[INFO] Server listening on {HOST}:{actual_port} ...")
        
        while True:
            conn, addr = s.accept()
            client_thread = threading.Thread(target=handle_client, args=(conn, addr), daemon=True)
            client_thread.start()

if __name__ == "__main__":
    print("[INFO] Starting server...")
    tcp_management()
